...
export const TWO_FACTOR_ISSUER = "Pannon Transfer VITESCO";

export const VITESCO_SHARED_DB_NAME = "pannontransferfoglalasikozpont";
...
let cachedVitescoCol: any = null;

export async function getVitescoCollection() {
  if (cachedVitescoCol) {
    return cachedVitescoCol;
  }
  // Ugyanazon a MongoClienten át, de KÖZÖS CRM DB-t használunk
  const { client } = await connectToDatabase();
  const db = client.db(VITESCO_SHARED_DB_NAME);
  const col = db.collection<VitescoPortalUser>(COLLECTION_NAME);
...
export async function findVitescoUserByInviteToken(rawToken: string, opts?: { allowExpired?: boolean }): Promise<VitescoPortalUser | null> {
  const col = await getVitescoCollection();
  ...
  return (u as VitescoPortalUser) || null;
}

export async function findVitescoUserByEmail(email: string): Promise<VitescoPortalUser | null> {
  const col = await getVitescoCollection();
  const normalized = normalizeEmail(email);
  return (await col.findOne({ normalizedEmail: normalized })) as VitescoPortalUser | null;
}

export async function setVitescoPasswordAndActivate(
  id: ObjectId,
  password: string
  ...
  return (await col.findOne({ _id: id })) as VitescoPortalUser | null;
}
...
export async function compareVitescoPassword(user: VitescoPortalUser, password: string): Promise<boolean> {
  if (!user.hashedPassword) return false;
  return bcrypt.compare(password, user.hashedPassword);
}

export async function recordVitescoLogin(id: ObjectId) {
  const col = await getVitescoCollection();
...
export async function findVitescoUserByMagicLoginToken(raw: string, allowExpired = false): Promise<VitescoPortalUser | null> {
  const col = await getVitescoCollection();
  ...
  return (await col.findOne(filter)) as VitescoPortalUser | null;
}

// ============ 2FA / TOTP ============
export function generateVitescoTwoFactorSecret(email: string): {
  secretBase32: string;
  uri: string;
...
export function verifyVitescoTwoFactorToken(
  user: Pick<VitescoPortalUser, "twoFactorSecret">,
  token: string
...
export async function setVitescoTwoFactorSecret(
  id: ObjectId,
...
export async function setVitescoTwoFactorEnabled(id: ObjectId, enabled: boolean) {
  const col = await getVitescoCollection();
  await col.updateOne(
    { _id: id },
...
export async function consumeVitescoTwoFactorBackupCode(id: ObjectId, code: string): Promise<boolean> {
  const col = await getVitescoCollection();
