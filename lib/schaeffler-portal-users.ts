...
export const TWO_FACTOR_ISSUER = "Pannon Transfer SCHAEFFLER";

export const SCHAEFFLER_SHARED_DB_NAME = "pannontransferfoglalasikozpont";
...
let cachedSchaefflerCol: any = null;

export async function getSchaefflerCollection() {
  if (cachedSchaefflerCol) {
    return cachedSchaefflerCol;
  }
  // Ugyanazon a MongoClienten át, de KÖZÖS CRM DB-t használunk
  const { client } = await connectToDatabase();
  const db = client.db(SCHAEFFLER_SHARED_DB_NAME);
  const col = db.collection<SchaefflerPortalUser>(COLLECTION_NAME);
...
export async function findSchaefflerUserByInviteToken(rawToken: string, opts?: { allowExpired?: boolean }): Promise<SchaefflerPortalUser | null> {
  ...
  return (u as SchaefflerPortalUser) || null;
}

export async function findSchaefflerUserByEmail(email: string): Promise<SchaefflerPortalUser | null> {
  const col = await getSchaefflerCollection();
  const normalized = normalizeEmail(email);
  return (await col.findOne({ normalizedEmail: normalized })) as SchaefflerPortalUser | null;
}

export async function setSchaefflerPasswordAndActivate(
  id: ObjectId,
  password: string
  ...
  return (await col.findOne({ _id: id })) as SchaefflerPortalUser | null;
}
...
export async function compareSchaefflerPassword(user: SchaefflerPortalUser, password: string): Promise<boolean> {
  if (!user.hashedPassword) return false;
  return bcrypt.compare(password, user.hashedPassword);
}

export async function recordSchaefflerLogin(id: ObjectId) {
  const col = await getSchaefflerCollection();
...
export async function findSchaefflerUserByMagicLoginToken(raw: string, allowExpired = false): Promise<SchaefflerPortalUser | null> {
  const col = await getSchaefflerCollection();
  ...
  return (await col.findOne(filter)) as SchaefflerPortalUser | null;
}

// ============ 2FA / TOTP ============
export function generateSchaefflerTwoFactorSecret(email: string): {
  secretBase32: string;
  uri: string;
...
export function verifySchaefflerTwoFactorToken(
  user: Pick<SchaefflerPortalUser, "twoFactorSecret">,
  token: string
...
export async function setSchaefflerTwoFactorSecret(
  id: ObjectId,
...
export async function setSchaefflerTwoFactorEnabled(id: ObjectId, enabled: boolean) {
  const col = await getSchaefflerCollection();
  await col.updateOne(
    { _id: id },
...
export async function consumeSchaefflerTwoFactorBackupCode(id: ObjectId, code: string): Promise<boolean> {
  const col = await getSchaefflerCollection();
