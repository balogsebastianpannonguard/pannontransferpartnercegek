...
export const TWO_FACTOR_ISSUER = "Pannon Transfer KRONES";

export const KRONES_SHARED_DB_NAME = "pannontransferfoglalasikozpont";
...
let cachedKronesCol: any = null;

export async function getKronesCollection() {
  if (cachedKronesCol) {
    return cachedKronesCol;
  }
  // Ugyanazon a MongoClienten át, de KÖZÖS CRM DB-t használunk
  const { client } = await connectToDatabase();
  const db = client.db(KRONES_SHARED_DB_NAME);
  const col = db.collection<KronesPortalUser>(COLLECTION_NAME);
...
export async function findKronesUserByInviteToken(rawToken: string, opts?: { allowExpired?: boolean }): Promise<KronesPortalUser | null> {
  ...
  return (u as KronesPortalUser) || null;
}

export async function findKronesUserByEmail(email: string): Promise<KronesPortalUser | null> {
  const col = await getKronesCollection();
  const normalized = normalizeEmail(email);
  return (await col.findOne({ normalizedEmail: normalized })) as KronesPortalUser | null;
}

export async function setKronesPasswordAndActivate(
  id: ObjectId,
  password: string
  ...
  return (await col.findOne({ _id: id })) as KronesPortalUser | null;
}
...
export async function compareKronesPassword(user: KronesPortalUser, password: string): Promise<boolean> {
  if (!user.hashedPassword) return false;
  return bcrypt.compare(password, user.hashedPassword);
}

export async function recordKronesLogin(id: ObjectId) {
  const col = await getKronesCollection();
...
export async function findKronesUserByMagicLoginToken(raw: string, allowExpired = false): Promise<KronesPortalUser | null> {
  const col = await getKronesCollection();
  ...
  return (await col.findOne(filter)) as KronesPortalUser | null;
}

// ============ 2FA / TOTP ============
export function generateKronesTwoFactorSecret(email: string): {
  secretBase32: string;
  uri: string;
...
export function verifyKronesTwoFactorToken(
  user: Pick<KronesPortalUser, "twoFactorSecret">,
  token: string
...
export async function setKronesTwoFactorSecret(
  id: ObjectId,
...
export async function setKronesTwoFactorEnabled(id: ObjectId, enabled: boolean) {
  const col = await getKronesCollection();
  await col.updateOne(
    { _id: id },
...
export async function consumeKronesTwoFactorBackupCode(id: ObjectId, code: string): Promise<boolean> {
  const col = await getKronesCollection();
