...
import { findSchaefflerUserByInviteToken, hashToken, SCHAEFFLER_SHARED_DB_NAME } from "@/lib/schaeffler-portal-users";
...
export async function POST(request: Request) {
  try {
    ...
    console.log("\n=== [SCHAEFFLER validate-invite] REQUEST ===");
    console.log("Shared DB:", SCHAEFFLER_SHARED_DB_NAME);
    console.log("Raw token:", peek);
    console.log("SHA256 hash:", hashPreview);
    // =========================================================================

    // 1) Valid (non-expired) token first
    let user = await findSchaefflerUserByInviteToken(rawToken);
    let expiredButActivated = false;

    // 2) Fallback: expired but already activated user — avoid "invalid link" error
    if (!user) {
      const maybeUser = await findSchaefflerUserByInviteToken(rawToken, {
        allowExpired: true,
      });
      if (maybeUser && maybeUser.isActivated) {
        ...
        console.log("[SCHAEFFLER validate-invite] NINCS TALÁLAT a közös DB-ben -> invalid link.");
        console.log("====================================================\n");
        return NextResponse.json(
    ...
    console.log("[SCHAEFFLER validate-invite] TALÁLAT:", {
      email: user.email,
    ...
    return NextResponse.json({
      ...
      hint: expiredButActivated
        ? "A link lejárt, de a fiókod már aktív. Lépj be a SCHAEFFLER Portálon keresztül."
        : null,
    });
  } catch (error) {
    console.error("[SCHAEFFLER validate-invite] ERROR:", error);
    return NextResponse.json(
