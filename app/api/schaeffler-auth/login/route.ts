...
import { authenticateSchaefflerByPassword, createSchaefflerSessionToken, setSchaefflerSessionCookie } from "@/lib/schaeffler-auth";
import { signSchaefflerLoginChallenge } from "@/lib/schaeffler-auth-login-challenge";
...
export async function POST(request: Request) {
  try {
    ...
    const result = await authenticateSchaefflerByPassword(String(email || ""), String(password || ""));
    ...
    if (user.requireTwoFactor && user.twoFactorEnabled) {
      const challengeToken = signSchaefflerLoginChallenge({
    ...
    const token = createSchaefflerSessionToken(user);
    await setSchaefflerSessionCookie(token, remember !== false);
    return NextResponse.json({
      success: true,
      needTwoFactor: false,
      user: {
