...
import {
  findSchaefflerUserByMagicLoginToken,
 ...
 * és ha valid → session cookie beállítása + redirect /schaeffler-re.
 */
export async function POST(request: Request) {
  try {
    ...
    const user = await findSchaefflerUserByMagicLoginToken(token, false);
    if (!user) {
      return NextResponse.json(
    ...
    if (useBackup) {
      valid = await consumeSchaefflerTwoFactorBackupCode(user._id as ObjectId, code);
      usedBackup = valid;
    } else {
      ...
      const v = verifySchaefflerTwoFactorToken(user, code);
      valid = !!v.valid;
    }
    ...
    await recordSchaefflerLogin(sid);
    const jwt = createSchaefflerSessionToken(user);
    await setSchaefflerSessionCookie(jwt, true);
