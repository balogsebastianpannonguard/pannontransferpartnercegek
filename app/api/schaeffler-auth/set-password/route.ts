...
import { setSchaefflerPasswordByInvite } from "@/lib/schaeffler-auth";
import {
  markSchaefflerWelcomeEmailSent,
  rotateMagicLoginToken,
  generateSchaefflerTwoFactorSecret,
  setSchaefflerTwoFactorSecret,
  TWO_FACTOR_ISSUER,
...
<tr><td bgcolor="#040914" style="padding:44px 40px; border-bottom:3px solid #009A44;">
...
<span style="font-family:-apple-system, sans-serif; font-size:22px; font-weight:900; color:#009A44;">SCHAEFFLER</span>
</td></tr></table>
...
<p style="margin:0; font-size:11px; font-weight:700; letter-spacing:3px; text-transform:uppercase; color:#60A5FA;">Pannon Transfer Schaeffler Portál</p>
</td></tr>
...
Köszöntjük a Schaeffler dedikált portálon. A fiókod mostantól aktív.
</p>
...
<td align="center" bgcolor="#009A44" style="border-radius:10px;">
...
Egyedi belépés a Schaeffler Portálra →
</a>
</td></tr></table>
</td></tr></table>
...
<p style="margin:0; font-size:14px; font-weight:700; color:#0F172A;">Schaeffler dedikált partner · ${twoFactorRequired ? "2FA kötelező" : "2FA nélkül"}</p>
</td>
</tr></table>
...
Ezt az üzenetet a Pannon Transfer Schaeffler Portál rendszere küldte.<br>
...
© ${new Date().getFullYear()} Pannon Transfer. Minden jog fenntartva.
  ...
  const text = [
    "Üdvözölünk a Schaeffler Portálon!",
    "",
    ...
    "",
    "Kizárólag az alábbi EGYEDI linken keresztül tudsz belépni a Schaeffler Portálra (publikus login nincs):",
    loginLink,
    "",
    ...
    "",
    "Üdvözlettel:",
    "Pannon Transfer - Schaeffler dedikált portál",
    `© ${new Date().getFullYear()} Pannon Transfer. Minden jog fenntartva.`,
  ].join("\n");
  return { html, text };
}
...
export async function POST(request: Request) {
  try {
    ...
    const res = await setSchaefflerPasswordByInvite(token, password);
    ...
    if (require2FA) {
      const tfa = generateSchaefflerTwoFactorSecret(res.user.email);
      twoFactorSetup = {
      ...
      await setSchaefflerTwoFactorSecret(userId, {
        secretBase32: tfa.secretBase32,
        backupCodes: tfa.backupCodes,
      });
    ...
    // 3) Welcome email küldés az EGYEDI magic login linkkel (nem publikus /schaeffler!)
    try {
      let origin = "";
      try {
      ...
      const magicLink = `${loginBase}/schaeffler/auth?token=${encodeURIComponent(magicRaw)}`;
      const { html, text } = buildWelcomeEmail(res.user.email, magicLink, require2FA);

      console.log("\n=========== SCHAEFFLER WELCOME EMAIL (TEST MODE) ===========");
      ...
      const emailRes = await sendEmail({
        to: res.user.email,
        subject: require2FA
          ? "Schaeffler Portál – Jelszó beállítva · 2FA kötelező · Egyedi belépési link"
          : "Schaeffler Portál – Üdvözlünk! Jelszó beállítva · Egyedi belépési link",
        html,
        text,
      });

      if (emailRes.success) {
        await markSchaefflerWelcomeEmailSent(userId);
      }
    } catch (err) {
      console.warn("[SCHAEFFLER welcome email] Nem küldhető el:", err);
    }

    // ⚠ FONTOS: NEM KAPUNK SESSION COOKIE-T a setup-password végén (történelmi beépítés ellen).
    // A belépés CSAK a welcome emailben küldött EGYEDI /schaeffler/auth?token= linken keresztül lehetséges.
