...
import { setEccoinoPasswordByInvite } from "@/lib/eccoino-auth";
import {
  markEccoinoWelcomeEmailSent,
  rotateMagicLoginToken,
  generateEccoinoTwoFactorSecret,
  setEccoinoTwoFactorSecret,
  TWO_FACTOR_ISSUER,
...
<tr><td bgcolor="#040914" style="padding:44px 40px; border-bottom:3px solid #60B8FF;">
...
<span style="font-family:-apple-system, sans-serif; font-size:22px; font-weight:900; color:#60B8FF;">ECCOINO</span>
</td></tr></table>
...
<p style="margin:0; font-size:11px; font-weight:700; letter-spacing:3px; text-transform:uppercase; color:#60A5FA;">Pannon Transfer Eccoino Portál</p>
</td></tr>
...
Köszöntjük a Eccoino dedikált portálon. A fiókod mostantól aktív.
</p>
...
<td align="center" bgcolor="#60B8FF" style="border-radius:10px;">
...
Egyedi belépés a Eccoino Portálra →
</a>
</td></tr></table>
</td></tr></table>
...
<p style="margin:0; font-size:14px; font-weight:700; color:#0F172A;">Eccoino dedikált partner · ${twoFactorRequired ? "2FA kötelező" : "2FA nélkül"}</p>
</td>
</tr></table>
...
Ezt az üzenetet a Pannon Transfer Eccoino Portál rendszere küldte.<br>
...
© ${new Date().getFullYear()} Pannon Transfer. Minden jog fenntartva.
  ...
  const text = [
    ...
    "",
    "Kizárólag az alábbi EGYEDI linken keresztül tudsz belépni a Eccoino Portálra (publikus login nincs):",
    loginLink,
    "",
    ...
    "",
    "Üdvözlettel:",
    "Pannon Transfer - Eccoino dedikált portál",
    `© ${new Date().getFullYear()} Pannon Transfer. Minden jog fenntartva.`,
  ].join("\n");
  return { html, text };
}
...
export async function POST(request: Request) {
  try {
    ...
    const res = await setEccoinoPasswordByInvite(token, password);
    ...
    if (require2FA) {
      const tfa = generateEccoinoTwoFactorSecret(res.user.email);
      twoFactorSetup = {
      ...
      await setEccoinoTwoFactorSecret(userId, {
        secretBase32: tfa.secretBase32,
        backupCodes: tfa.backupCodes,
      });
    ...
    // 3) Welcome email küldés az EGYEDI magic login linkkel (nem publikus /eccoino!)
    try {
      let origin = "";
      try {
      ...
      const magicLink = `${loginBase}/eccoino/auth?token=${encodeURIComponent(magicRaw)}`;
      const { html, text } = buildWelcomeEmail(res.user.email, magicLink, require2FA);

      console.log("\n=========== ECCOINO WELCOME EMAIL (TEST MODE) ===========");
      ...
      const emailRes = await sendEmail({
        to: res.user.email,
        subject: require2FA
          ? "Eccoino Portál – Jelszó beállítva · 2FA kötelező · Egyedi belépési link"
          : "Eccoino Portál – Üdvözlünk! Jelszó beállítva · Egyedi belépési link",
        html,
        text,
      });

      if (emailRes.success) {
        await markEccoinoWelcomeEmailSent(userId);
      }
    } catch (err) {
      console.warn("[ECCOINO welcome email] Nem küldhető el:", err);
    }

    // ⚠ FONTOS: NEM KAPUNK SESSION COOKIE-T a setup-password végén (történelmi beépítés ellen).
    // A belépés CSAK a welcome emailben küldött EGYEDI /eccoino/auth?token= linken keresztül lehetséges.
