...
import { findEccoinoUserByInviteToken, hashToken, ECCOINO_SHARED_DB_NAME } from "@/lib/eccoino-portal-users";
  ...
  try {
    ...
    if (!user) {
      ...
      if (maybeUser && maybeUser.isActivated) {
        ...
        console.log("[ECCOINO validate-invite] NINCS TALÁLAT a közös DB-ben -> invalid link.");
        console.log("====================================================\n");
        return NextResponse.json(
    ...
    console.log("[ECCOINO validate-invite] TALÁLAT:", {
      email: user.email,
    ...
    return NextResponse.json({
      ...
      hint: expiredButActivated
        ? "A link lejárt, de a fiókod már aktív. Lépj be a ECCOINO Portálon keresztül."
        : null,
    });
  } catch (error) {
    console.error("[ECCOINO validate-invite] ERROR:", error);
    return NextResponse.json(
