...
import { verifyEccoinoLoginChallenge } from "@/lib/eccoino-auth-login-challenge";
import {
  getEccoinoCollection,
...
export async function POST(request: Request) {
  try {
    ...
    const challenge = verifyEccoinoLoginChallenge(challengeToken);
    if (!challenge) {
      return NextResponse.json(
    ...
    let ok = false;
    if (useBackup) {
      ok = await consumeEccoinoTwoFactorBackupCode(user._id!, cleanCode);
    } else {
      const res = verifyEccoinoTwoFactorToken(user, cleanCode);
      ok = !!res.valid;
    }
    ...
    const sessionToken = createEccoinoSessionToken(user);
    await setEccoinoSessionCookie(sessionToken, true);
