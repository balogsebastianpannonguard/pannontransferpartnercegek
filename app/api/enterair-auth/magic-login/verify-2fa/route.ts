...
import {
  findEnterairUserByMagicLoginToken,
 ...
 * és ha valid → session cookie beállítása + redirect /enterair-re.
 */
export async function POST(request: Request) {
  try {
    ...
    const user = await findEnterairUserByMagicLoginToken(token, false);
    if (!user) {
      return NextResponse.json(
    ...
    if (useBackup) {
      valid = await consumeEnterairTwoFactorBackupCode(user._id as ObjectId, code);
      usedBackup = valid;
    } else {
      ...
      const v = verifyEnterairTwoFactorToken(user, code);
      valid = !!v.valid;
    }
    ...
    await recordEnterairLogin(sid);
    const jwt = createEnterairSessionToken(user);
    await setEnterairSessionCookie(jwt, true);
