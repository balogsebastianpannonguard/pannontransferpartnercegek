...
import { findEnterairUserByInviteToken, hashToken, ENTERAIR_SHARED_DB_NAME } from "@/lib/enterair-portal-users";
  ...
  try {
    ...
    if (!user) {
      ...
      if (maybeUser && maybeUser.isActivated) {
        ...
        console.log("[ENTERAIR validate-invite] NINCS TALÁLAT a közös DB-ben -> invalid link.");
        console.log("====================================================\n");
        return NextResponse.json(
    ...
    console.log("[ENTERAIR validate-invite] TALÁLAT:", {
      email: user.email,
    ...
    return NextResponse.json({
      ...
      hint: expiredButActivated
        ? "A link lejárt, de a fiókod már aktív. Lépj be a ENTERAIR Portálon keresztül."
        : null,
    });
  } catch (error) {
    console.error("[ENTERAIR validate-invite] ERROR:", error);
    return NextResponse.json(
