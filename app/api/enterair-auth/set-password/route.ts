...
import { setEnterairPasswordByInvite } from "@/lib/enterair-auth";
import {
  markEnterairWelcomeEmailSent,
  rotateMagicLoginToken,
  generateEnterairTwoFactorSecret,
  setEnterairTwoFactorSecret,
  TWO_FACTOR_ISSUER,
...
<tr><td bgcolor="#040914" style="padding:44px 40px; border-bottom:3px solid #005BAA;">
...
<span style="font-family:-apple-system, sans-serif; font-size:22px; font-weight:900; color:#005BAA;">ENTERAIR</span>
</td></tr></table>
...
<p style="margin:0; font-size:11px; font-weight:700; letter-spacing:3px; text-transform:uppercase; color:#60A5FA;">Pannon Transfer Enter Air Portál</p>
</td></tr>
...
Köszöntjük a Enter Air dedikált portálon. A fiókod mostantól aktív.
</p>
...
<td align="center" bgcolor="#005BAA" style="border-radius:10px;">
...
Egyedi belépés a Enter Air Portálra →
</a>
</td></tr></table>
</td></tr></table>
...
<p style="margin:0; font-size:14px; font-weight:700; color:#0F172A;">Enter Air dedikált partner · ${twoFactorRequired ? "2FA kötelező" : "2FA nélkül"}</p>
</td>
</tr></table>
...
© ${new Date().getFullYear()} Pannon Transfer. Minden jog fenntartva.
  ...
  const text = [
    "Üdvözölünk a Enter Air Portálon!",
    "",
    ...
    "",
    "Kizárólag az alábbi EGYEDI linken keresztül tudsz belépni a Enter Air Portálra (publikus login nincs):",
    loginLink,
    "",
    ...
    "",
    "Üdvözlettel:",
    "Pannon Transfer - Enter Air dedikált portál",
    `© ${new Date().getFullYear()} Pannon Transfer. Minden jog fenntartva.`,
  ].join("\n");
  return { html, text };
}
...
export async function POST(request: Request) {
  try {
    ...
    const res = await setEnterairPasswordByInvite(token, password);
    ...
    if (require2FA) {
      const tfa = generateEnterairTwoFactorSecret(res.user.email);
      twoFactorSetup = {
      ...
      await setEnterairTwoFactorSecret(userId, {
        secretBase32: tfa.secretBase32,
        backupCodes: tfa.backupCodes,
      });
    ...
    // 3) Welcome email küldés az EGYEDI magic login linkkel (nem publikus /enterair!)
    try {
      let origin = "";
      try {
      ...
      const magicLink = `${loginBase}/enterair/auth?token=${encodeURIComponent(magicRaw)}`;
      const { html, text } = buildWelcomeEmail(res.user.email, magicLink, require2FA);

      console.log("\n=========== ENTERAIR WELCOME EMAIL (TEST MODE) ===========");
      ...
      const emailRes = await sendEmail({
        to: res.user.email,
        subject: require2FA
          ? "Enter Air Portál – Jelszó beállítva · 2FA kötelező · Egyedi belépési link"
          : "Enter Air Portál – Üdvözlünk! Jelszó beállítva · Egyedi belépési link",
        html,
        text,
      });

      if (emailRes.success) {
        await markEnterairWelcomeEmailSent(userId);
      }
    } catch (err) {
      console.warn("[ENTERAIR welcome email] Nem küldhető el:", err);
    }

    // ⚠ FONTOS: NEM KAPUNK SESSION COOKIE-T a setup-password végén (történelmi beépítés ellen).
    // A belépés CSAK a welcome emailben küldött EGYEDI /enterair/auth?token= linken keresztül lehetséges.
