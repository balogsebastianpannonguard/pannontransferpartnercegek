...
import { setEcoproPasswordByInvite } from "@/lib/ecopro-auth";
import {
  markEcoproWelcomeEmailSent,
  rotateMagicLoginToken,
  generateEcoproTwoFactorSecret,
  setEcoproTwoFactorSecret,
  TWO_FACTOR_ISSUER,
...
<tr><td bgcolor="#040914" style="padding:44px 40px; border-bottom:3px solid #0096D6;">
...
<span style="font-family:-apple-system, sans-serif; font-size:22px; font-weight:900; color:#0096D6;">ECOPRO</span>
</td></tr></table>
...
<p style="margin:0; font-size:11px; font-weight:700; letter-spacing:3px; text-transform:uppercase; color:#60A5FA;">Pannon Transfer EcoPro Portál</p>
</td></tr>
...
Köszöntjük a EcoPro dedikált portálon. A fiókod mostantól aktív.
</p>
...
<td align="center" bgcolor="#0096D6" style="border-radius:10px;">
...
Egyedi belépés a EcoPro Portálra →
</a>
</td></tr></table>
</td></tr></table>
...
<p style="margin:0; font-size:14px; font-weight:700; color:#0F172A;">EcoPro dedikált partner · ${twoFactorRequired ? "2FA kötelező" : "2FA nélkül"}</p>
</td>
</tr></table>
...
Ezt az üzenetet a Pannon Transfer EcoPro Portál rendszere küldte.<br>
...
© ${new Date().getFullYear()} Pannon Transfer. Minden jog fenntartva.
  ...
  const text = [
    ...
    "",
    "Kizárólag az alábbi EGYEDI linken keresztül tudsz belépni a EcoPro Portálra (publikus login nincs):",
    loginLink,
    "",
    ...
    "",
    "Üdvözlettel:",
    "Pannon Transfer - EcoPro dedikált portál",
    `© ${new Date().getFullYear()} Pannon Transfer. Minden jog fenntartva.`,
  ].join("\n");
  return { html, text };
}
...
export async function POST(request: Request) {
  try {
    ...
    const res = await setEcoproPasswordByInvite(token, password);
    ...
    if (require2FA) {
      ...
      await setEcoproTwoFactorSecret(userId, {
        secretBase32: tfa.secretBase32,
        backupCodes: tfa.backupCodes,
      });
    ...
    // 3) Welcome email küldés az EGYEDI magic login linkkel (nem publikus /ecopro!)
    try {
      let origin = "";
      try {
      ...
      const magicLink = `${loginBase}/ecopro/auth?token=${encodeURIComponent(magicRaw)}`;
      const { html, text } = buildWelcomeEmail(res.user.email, magicLink, require2FA);

      console.log("\n=========== ECOPRO WELCOME EMAIL (TEST MODE) ===========");
      ...
      const emailRes = await sendEmail({
        to: res.user.email,
        subject: require2FA
          ? "EcoPro Portál – Jelszó beállítva · 2FA kötelező · Egyedi belépési link"
          : "EcoPro Portál – Üdvözlünk! Jelszó beállítva · Egyedi belépési link",
        html,
        text,
      });

      if (emailRes.success) {
        await markEcoproWelcomeEmailSent(userId);
      }
    } catch (err) {
      console.warn("[ECOPRO welcome email] Nem küldhető el:", err);
    }

    // ⚠ FONTOS: NEM KAPUNK SESSION COOKIE-T a setup-password végén (történelmi beépítés ellen).
    // A belépés CSAK a welcome emailben küldött EGYEDI /ecopro/auth?token= linken keresztül lehetséges.
