...
import { findEcoproUserByInviteToken, hashToken, ECOPRO_SHARED_DB_NAME } from "@/lib/ecopro-portal-users";
  ...
  try {
    ...
    if (!user) {
      ...
      if (maybeUser && maybeUser.isActivated) {
        ...
        console.log("[ECOPRO validate-invite] NINCS TALÁLAT a közös DB-ben -> invalid link.");
        console.log("====================================================\n");
        return NextResponse.json(
    ...
    console.log("[ECOPRO validate-invite] TALÁLAT:", {
      email: user.email,
    ...
    return NextResponse.json({
      ...
      hint: expiredButActivated
        ? "A link lejárt, de a fiókod már aktív. Lépj be a ECOPRO Portálon keresztül."
        : null,
    });
  } catch (error) {
    console.error("[ECOPRO validate-invite] ERROR:", error);
    return NextResponse.json(
