...
import { authenticateTamaByPassword, createTamaSessionToken, setTamaSessionCookie } from "@/lib/tama-auth";
import { signTamaLoginChallenge } from "@/lib/tama-auth-login-challenge";
...
export async function POST(request: Request) {
  try {
    ...
    const result = await authenticateTamaByPassword(String(email || ""), String(password || ""));
    ...
    if (user.requireTwoFactor && user.twoFactorEnabled) {
      const challengeToken = signTamaLoginChallenge({
    ...
    const token = createTamaSessionToken(user);
    await setTamaSessionCookie(token, remember !== false);
    return NextResponse.json({
      success: true,
      needTwoFactor: false,
      user: {
