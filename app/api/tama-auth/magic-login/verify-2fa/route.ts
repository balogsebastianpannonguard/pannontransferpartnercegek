 ...
 * és ha valid → session cookie beállítása + redirect /tama-re.
 */
export async function POST(request: Request) {
  try {
    ...
    const user = await findTamaUserByMagicLoginToken(token, false);
    if (!user) {
      return NextResponse.json(
    ...
    if (useBackup) {
      valid = await consumeTamaTwoFactorBackupCode(user._id as ObjectId, code);
      usedBackup = valid;
    } else {
      ...
      const v = verifyTamaTwoFactorToken(user, code);
      valid = !!v.valid;
    }
    ...
    await recordTamaLogin(sid);
    const jwt = createTamaSessionToken(user);
    await setTamaSessionCookie(jwt, true);
