...
import {
  findKronesUserByMagicLoginToken,
 ...
 * és ha valid → session cookie beállítása + redirect /krones-re.
 */
export async function POST(request: Request) {
  try {
    ...
    const user = await findKronesUserByMagicLoginToken(token, false);
    if (!user) {
      return NextResponse.json(
    ...
    if (useBackup) {
      valid = await consumeKronesTwoFactorBackupCode(user._id as ObjectId, code);
      usedBackup = valid;
    } else {
      ...
      const v = verifyKronesTwoFactorToken(user, code);
      valid = !!v.valid;
    }
    ...
    await recordKronesLogin(sid);
    const jwt = createKronesSessionToken(user);
    await setKronesSessionCookie(jwt, true);
