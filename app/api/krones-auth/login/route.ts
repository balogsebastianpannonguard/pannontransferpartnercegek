...
import { authenticateKronesByPassword, createKronesSessionToken, setKronesSessionCookie } from "@/lib/krones-auth";
import { signKronesLoginChallenge } from "@/lib/krones-auth-login-challenge";
...
export async function POST(request: Request) {
  try {
    ...
    const result = await authenticateKronesByPassword(String(email || ""), String(password || ""));
    ...
    if (user.requireTwoFactor && user.twoFactorEnabled) {
      const challengeToken = signKronesLoginChallenge({
    ...
    const token = createKronesSessionToken(user);
    await setKronesSessionCookie(token, remember !== false);
    return NextResponse.json({
      success: true,
      needTwoFactor: false,
      user: {
