...
import { setKronesPasswordByInvite } from "@/lib/krones-auth";
import {
  markKronesWelcomeEmailSent,
  rotateMagicLoginToken,
  generateKronesTwoFactorSecret,
  setKronesTwoFactorSecret,
  TWO_FACTOR_ISSUER,
...
<tr><td bgcolor="#040914" style="padding:44px 40px; border-bottom:3px solid #003F8A;">
...
<span style="font-family:-apple-system, sans-serif; font-size:22px; font-weight:900; color:#003F8A;">KRONES</span>
</td></tr></table>
...
<p style="margin:0; font-size:11px; font-weight:700; letter-spacing:3px; text-transform:uppercase; color:#60A5FA;">Pannon Transfer Krones Portál</p>
</td></tr>
...
Köszöntjük a Krones dedikált portálon. A fiókod mostantól aktív.
</p>
...
<td align="center" bgcolor="#003F8A" style="border-radius:10px;">
...
Egyedi belépés a Krones Portálra →
</a>
</td></tr></table>
</td></tr></table>
...
<p style="margin:0; font-size:14px; font-weight:700; color:#0F172A;">Krones dedikált partner · ${twoFactorRequired ? "2FA kötelező" : "2FA nélkül"}</p>
</td>
</tr></table>
...
Ezt az üzenetet a Pannon Transfer Krones Portál rendszere küldte.<br>
...
© ${new Date().getFullYear()} Pannon Transfer. Minden jog fenntartva.
  ...
  const text = [
    ...
    "",
    "Kizárólag az alábbi EGYEDI linken keresztül tudsz belépni a Krones Portálra (publikus login nincs):",
    loginLink,
    "",
    ...
    "",
    "Üdvözlettel:",
    "Pannon Transfer - Krones dedikált portál",
    `© ${new Date().getFullYear()} Pannon Transfer. Minden jog fenntartva.`,
  ].join("\n");
  return { html, text };
}
...
export async function POST(request: Request) {
  try {
    ...
    const res = await setKronesPasswordByInvite(token, password);
    ...
    if (require2FA) {
      const tfa = generateKronesTwoFactorSecret(res.user.email);
      twoFactorSetup = {
      ...
      await setKronesTwoFactorSecret(userId, {
        secretBase32: tfa.secretBase32,
        backupCodes: tfa.backupCodes,
      });
    ...
    // 3) Welcome email küldés az EGYEDI magic login linkkel (nem publikus /krones!)
    try {
      let origin = "";
      try {
      ...
      const magicLink = `${loginBase}/krones/auth?token=${encodeURIComponent(magicRaw)}`;
      const { html, text } = buildWelcomeEmail(res.user.email, magicLink, require2FA);

      console.log("\n=========== KRONES WELCOME EMAIL (TEST MODE) ===========");
      ...
      const emailRes = await sendEmail({
        to: res.user.email,
        subject: require2FA
          ? "Krones Portál – Jelszó beállítva · 2FA kötelező · Egyedi belépési link"
          : "Krones Portál – Üdvözlünk! Jelszó beállítva · Egyedi belépési link",
        html,
        text,
      });

      if (emailRes.success) {
        await markKronesWelcomeEmailSent(userId);
      }
    } catch (err) {
      console.warn("[KRONES welcome email] Nem küldhető el:", err);
    }

    // ⚠ FONTOS: NEM KAPUNK SESSION COOKIE-T a setup-password végén (történelmi beépítés ellen).
    // A belépés CSAK a welcome emailben küldött EGYEDI /krones/auth?token= linken keresztül lehetséges.
