...
import { findKronesUserByInviteToken, hashToken, KRONES_SHARED_DB_NAME } from "@/lib/krones-portal-users";
  ...
  try {
    ...
    if (!user) {
      ...
      if (maybeUser && maybeUser.isActivated) {
        ...
        console.log("[KRONES validate-invite] NINCS TALÁLAT a közös DB-ben -> invalid link.");
        console.log("====================================================\n");
        return NextResponse.json(
    ...
    console.log("[KRONES validate-invite] TALÁLAT:", {
      email: user.email,
    ...
    return NextResponse.json({
      ...
      hint: expiredButActivated
        ? "A link lejárt, de a fiókod már aktív. Lépj be a KRONES Portálon keresztül."
        : null,
    });
  } catch (error) {
    console.error("[KRONES validate-invite] ERROR:", error);
    return NextResponse.json(
