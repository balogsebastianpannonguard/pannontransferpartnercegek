...
import {
  findVitescoUserByMagicLoginToken,
 ...
 * és ha valid → session cookie beállítása + redirect /vitesco-re.
 */
export async function POST(request: Request) {
  try {
    ...
    const user = await findVitescoUserByMagicLoginToken(token, false);
    if (!user) {
      return NextResponse.json(
    ...
    if (useBackup) {
      valid = await consumeVitescoTwoFactorBackupCode(user._id as ObjectId, code);
      usedBackup = valid;
    } else {
      ...
      const v = verifyVitescoTwoFactorToken(user, code);
      valid = !!v.valid;
    }
    ...
    await recordVitescoLogin(sid);
    const jwt = createVitescoSessionToken(user);
    await setVitescoSessionCookie(jwt, true);
