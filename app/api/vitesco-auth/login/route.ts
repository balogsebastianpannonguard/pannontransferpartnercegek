...
import { authenticateVitescoByPassword, createVitescoSessionToken, setVitescoSessionCookie } from "@/lib/vitesco-auth";
import { signVitescoLoginChallenge } from "@/lib/vitesco-auth-login-challenge";
...
export async function POST(request: Request) {
  try {
    ...
    const result = await authenticateVitescoByPassword(String(email || ""), String(password || ""));
    ...
    if (user.requireTwoFactor && user.twoFactorEnabled) {
      const challengeToken = signVitescoLoginChallenge({
    ...
    const token = createVitescoSessionToken(user);
    await setVitescoSessionCookie(token, remember !== false);
    return NextResponse.json({
      success: true,
      needTwoFactor: false,
      user: {
