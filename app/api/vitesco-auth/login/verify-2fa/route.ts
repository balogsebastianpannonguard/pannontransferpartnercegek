...
import { createVitescoSessionToken, setVitescoSessionCookie } from "@/lib/vitesco-auth";
import { verifyVitescoLoginChallenge } from "@/lib/vitesco-auth-login-challenge";
import {
  getVitescoCollection,
  verifyVitescoTwoFactorToken,
  consumeVitescoTwoFactorBackupCode,
  recordVitescoLogin,
...
export async function POST(request: Request) {
  try {
    ...
    const challenge = verifyVitescoLoginChallenge(challengeToken);
    if (!challenge) {
      return NextResponse.json(
    ...
    let ok = false;
    if (useBackup) {
      ok = await consumeVitescoTwoFactorBackupCode(user._id!, cleanCode);
    } else {
      const res = verifyVitescoTwoFactorToken(user, cleanCode);
      ok = !!res.valid;
    }
    ...
    const sessionToken = createVitescoSessionToken(user);
    await setVitescoSessionCookie(sessionToken, true);
