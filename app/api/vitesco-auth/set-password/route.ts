...
import { setVitescoPasswordByInvite } from "@/lib/vitesco-auth";
import {
  markVitescoWelcomeEmailSent,
  rotateMagicLoginToken,
  generateVitescoTwoFactorSecret,
  setVitescoTwoFactorSecret,
  TWO_FACTOR_ISSUER,
...
<tr><td bgcolor="#040914" style="padding:44px 40px; border-bottom:3px solid #E30613;">
...
<span style="font-family:-apple-system, sans-serif; font-size:22px; font-weight:900; color:#E30613;">VITESCO</span>
</td></tr></table>
...
<p style="margin:0; font-size:11px; font-weight:700; letter-spacing:3px; text-transform:uppercase; color:#60A5FA;">Pannon Transfer Vitesco Portál</p>
</td></tr>
...
Köszöntjük a Vitesco dedikált portálon. A fiókod mostantól aktív.
</p>
...
<td align="center" bgcolor="#E30613" style="border-radius:10px;">
...
Egyedi belépés a Vitesco Portálra →
</a>
</td></tr></table>
</td></tr></table>
...
<p style="margin:0; font-size:14px; font-weight:700; color:#0F172A;">Vitesco dedikált partner · ${twoFactorRequired ? "2FA kötelező" : "2FA nélkül"}</p>
</td>
</tr></table>
...
Ezt az üzenetet a Pannon Transfer Vitesco Portál rendszere küldte.<br>
...
© ${new Date().getFullYear()} Pannon Transfer. Minden jog fenntartva.
  ...
  const text = [
    ...
    "",
    "Kizárólag az alábbi EGYEDI linken keresztül tudsz belépni a Vitesco Portálra (publikus login nincs):",
    loginLink,
    "",
    ...
    "",
    "Üdvözlettel:",
    "Pannon Transfer - Vitesco dedikált portál",
    `© ${new Date().getFullYear()} Pannon Transfer. Minden jog fenntartva.`,
  ].join("\n");
  return { html, text };
}
...
export async function POST(request: Request) {
  try {
    ...
    const res = await setVitescoPasswordByInvite(token, password);
    ...
    if (require2FA) {
      const tfa = generateVitescoTwoFactorSecret(res.user.email);
      twoFactorSetup = {
      ...
      await setVitescoTwoFactorSecret(userId, {
        secretBase32: tfa.secretBase32,
        backupCodes: tfa.backupCodes,
      });
    ...
    // 3) Welcome email küldés az EGYEDI magic login linkkel (nem publikus /vitesco!)
    try {
      let origin = "";
      try {
      ...
      const magicLink = `${loginBase}/vitesco/auth?token=${encodeURIComponent(magicRaw)}`;
      const { html, text } = buildWelcomeEmail(res.user.email, magicLink, require2FA);

      console.log("\n=========== VITESCO WELCOME EMAIL (TEST MODE) ===========");
      ...
      const emailRes = await sendEmail({
        to: res.user.email,
        subject: require2FA
          ? "Vitesco Portál – Jelszó beállítva · 2FA kötelező · Egyedi belépési link"
          : "Vitesco Portál – Üdvözlünk! Jelszó beállítva · Egyedi belépési link",
        html,
        text,
      });

      if (emailRes.success) {
        await markVitescoWelcomeEmailSent(userId);
      }
    } catch (err) {
      console.warn("[VITESCO welcome email] Nem küldhető el:", err);
    }

    // ⚠ FONTOS: NEM KAPUNK SESSION COOKIE-T a setup-password végén (történelmi beépítés ellen).
    // A belépés CSAK a welcome emailben küldött EGYEDI /vitesco/auth?token= linken keresztül lehetséges.
