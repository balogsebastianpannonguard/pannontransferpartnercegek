...
export default function EcoproAuthPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch("/api/ecopro-auth/magic-login", {
          method: "POST",
        ...
        if (!res.ok || !json?.success) {
          ...
          setState("done");
          setTimeout(() => router.push("/ecopro"), 2500);
        }
      } catch {
        if (!cancelled) {
          setState("invalid");
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/ecopro-auth/magic-login/verify-2fa", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          token: rawToken,
        ...
        <div className="w-full max-w-[440px]">
          <AnimatePresence mode="wait">
            ...
            {state === "invalid" && (
              ...
              >
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <a
                    href="mailto:balog.sebastian@pannonguard.hu?subject=ECOPRO%20Port%C3%A1l%20-%20%C3%9Aj%20bel%C3%A9p%C3%A9si%20link%20k%C3%A9r%C3%A9se"
            ...
            {state === "twofactor" && (
              ...
              >
                <div className="mb-7">
                  <div className="mx-auto w-14 h-14 rounded-2xl bg-[#0096D6]/[0.08] border border-[#0096D6]/10 flex items-center justify-center mb-5">
                ...
                <div className="mb-4">
                  ...
                  <input
                    ...
                    className="w-full h-[52px] px-4 bg-white border border-zinc-200 rounded-xl text-[18px] text-center tracking-[0.45em] text-zinc-900 placeholder:text-zinc-400 font-mono font-bold focus:outline-none focus:ring-2 focus:ring-[#0096D6]/20 focus:border-[#0096D6]/50 transition-all"
                  />
                </div>
