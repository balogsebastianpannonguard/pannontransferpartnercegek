...
export default function CatlLandingPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch("/api/catl-auth/session", {
          cache: "no-store",
        });
        if (!active) return;
        if (res.ok) {
        ...
        <div className="w-full border-t border-zinc-200/70 bg-white/60 backdrop-blur-md mt-auto">
          <div className="max-w-6xl mx-auto px-6 h-14 flex items-center justify-between text-[11px] text-zinc-400 font-medium">
            ...
            <span className="tracking-wider">
              CATL Dedikált Ügyfélportál · Kizárólagos linkalapú hozzáférés
            </span>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-[1440px] mx-auto px-6 lg:px-12 h-20 flex justify-between items-center">
          ...
          <div className="hidden lg:flex items-center gap-8 h-full">
            <div className="flex items-center gap-8 h-full">
              <Link
                href="#booking"
                className={`text-sm font-medium tracking-wide hover:text-white transition-colors h-full flex items-center border-b-2 ${pathname === "/catl/bookings" ? "text-slate-400 border-transparent" : "text-slate-300 border-[#0047BA]"}`}
              >
                {t("nav", "booking")}
              </Link>
              <Link
                href="/catl/bookings"
                className={`text-sm font-medium tracking-wide hover:text-white transition-colors h-full flex items-center border-b-2 ${pathname === "/catl/bookings" ? "text-white border-[#0047BA]" : "text-slate-300 border-transparent hover:border-white/20"}`}
              >
                Foglalásaim
              </Link>
            </div>
            ...
            <div className="flex items-center gap-3 pl-2">
              <div className="hidden xl:flex flex-col items-end">
                ...
                <Link
                  href="/catl/bookings"
                  className="text-[10px] font-medium text-[#00B4D8] hover:text-[#0047BA] transition-colors mt-1 flex items-center gap-1"
                >
              ...
              <div className="p-8 md:p-12 space-y-10">
                ...
                <div className="space-y-6">
                  <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-800">
                    <div className="w-8 h-8 rounded-full bg-[#0047BA]/20 flex items-center justify-center text-[#00B4D8] font-bold text-sm border border-[#0047BA]/30">
                      1
                    </div>
